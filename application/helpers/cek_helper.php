...
{
  $ci = &get_instance();
  $session = $ci->session->userdata('id_operator');
  if ($session == '') {
    redirect(base_url());
  }
}
...
/**
 * KONFIGURASI LEVEL HARDCODED
 * level id 1: Super Admin
 * level id 2: Admin Data
 * level id 3: CS
 */

// jika session level = cs, kembalikan ke home
function cek_session_cs()
{
  $ci = &get_instance();
...
// jika session level = admin data, kembalikan ke home
function cek_session_admin_data()
{
  $ci = &get_instance();
  $session = $ci->session->userdata('id_level');
  if ($session == '2') {
...
function tgl_waktu_standard($tgl)
{
  $tanggal = substr($tgl, 0, 2);
  $bulan = substr($tgl, 3, 2);
  $tahun = substr($tgl, 6, 9);
